...
/**
 * {@link ClauseDetector} contains the methods dealing with the detection of clauses. After the detection is performed a
 * set of {@link Clause} is created.
 * 
 * {@code detectClauses} first detects the type of clause to be generated based on syntactic relations and once a clause
 * is detected a given method is used to create a {@link Clause}.
 * 
...
class ClauseDetector {
	 ...
	 * 
	 * @param roots
	 *            The list of roots of the clauses in the sentence
	 * @param clauses
	 *            The list of clauses in the sentence
	 * @param subject
	 *            The subject of the clause
	 * @param clauseRoot
	 *            The root of the clause, either a verb or a complement
	 * @param partmod
	 *            Indicates if the clause is generated from a partmod relation
	 */
	private static void addNsubjClause(ClausIE clausIE, List<IndexedWord> roots, List<Clause> clauses,
			IndexedWord subject, IndexedWord clauseRoot, boolean partmod) {
		...
		for (int i = 0; i < ccs.size(); i++) {
			...
			if (cop != null) {
				...
				clause.verb = clause.constituents.size();
				if (!partmod) {
					constRoot = new IndexedConstituent(semanticGraph, root, Collections.<IndexedWord> emptySet(),
							exclude, Constituent.Type.VERB);
				} else {
			...
			if (subject.tag().charAt(0) == 'W' && rcmod != null) {
				...
				if (sub != null)
					clause.constituents
							.add(new IndexedConstituent(semanticGraph, sub.getDependent(), Constituent.Type.SUBJECT));
				else
			...
			for (SemanticGraphEdge outgoingEdge : outgoingEdges) {
				...
				if (DpUtils.isComplm(outgoingEdge) || DpUtils.isMark(outgoingEdge)) {
					...
					if (dependent.tag().charAt(0) == 'W' && rcmod != null) {
						...
						clause.constituents
								.add(createPossConstituent(semanticGraph, poss, rcmod, dependent, Type.IOBJ));
						rcmod = null;
						// "regular case"
					} else
						clause.constituents
								.add(new IndexedConstituent(semanticGraph, dependent, Constituent.Type.IOBJ));
					// Direct Object
					...
					if (dependent.tag().charAt(0) == 'W' && rcmod != null) {
						...
						clause.constituents
								.add(createPossConstituent(semanticGraph, poss, rcmod, dependent, Type.DOBJ));
						rcmod = null;
					} else
						clause.constituents
								.add(new IndexedConstituent(semanticGraph, dependent, Constituent.Type.DOBJ));
					// CCOMPS
					...
					// XCOMPS (Note: Need special treatment, they won't form a new clause so optional/obligatory
					// constituents
					...
					clause.constituents
							.add(new XcompConstituent(semanticGraph, dependent, Constituent.Type.XCOMP, xcompclauses));
					...
					clause.constituents
							.add(new IndexedConstituent(semanticGraph, dependent, Constituent.Type.ADVERBIAL));
					// Rel appears in certain cases when relative pronouns act as prepositional objects "I saw the house
					// in which I grew".
			...
			// ------------------------To process relative clauses with implicit (zero) relative
			// pronoun-------------------------
			if (rcmod != null) { // "I saw the house I grew up in", "I saw
				...
				if (candidate != null) {
					...
					IndexedConstituent tmpconst = createRelConstituent(newSemanticGraph, rcmod.getGovernor(),
							Type.ADVERBIAL);
	 ...
	 * 
	 * @param outgoingEdge
	 *            The rel labeled edge
	 * @param semanticGraph
	 *            The semantic graph
	 * @param dependent
	 *            The dependent of the relation
	 * @param rcmod
	 *            The relative clause modifier of the relation refered by rel
	 * @param clause
	 *            A clause
	 */
	 ...
	 *            The semantic graph
	 * @param poss
	 *            The edge referring to the possessive relation
	 * @param rcmod
	 *            The relative clause modifier of the relation
	 * @param constGovernor
	 *            The root of the constituent
	 * @param type
	 *            The type of the constituent
	 */
	private static Constituent createPossConstituent(SemanticGraph semanticGraph, SemanticGraphEdge poss,
			SemanticGraphEdge rcmod, IndexedWord constGovernor, Type type) {
		...
		return new IndexedConstituent(newSemanticGraph, constGovernor, Collections.<IndexedWord> emptySet(), exclude,
				type);
	}
	...
	private static IndexedConstituent createRelConstituent(SemanticGraph semanticGraph, IndexedWord root, Type type) {
		...
		if (rccop != null) {
			...
			return new IndexedConstituent(semanticGraph, root, Collections.<IndexedWord> emptySet(), excludercmod,
					type);
		} else
	 ...
	 * 
	 * @param subject
	 *            The subject of the clause (first argument of the appos relation)
	 * @param object
	 *            The object of the clause (second argument of the appos relation)
	 */
	...
	private static void addPossessiveClause(ClausIE clausIE, IndexedWord subject, IndexedWord object) {
		...
		if (rcmod != null) {
			clause.constituents.add(createRelConstituent(newSemanticGraph, rcmod.getGovernor(), Type.SUBJECT));
			((IndexedConstituent) clause.constituents.get(0)).getExcludedVertexes().addAll(excludesub); // to avoid the
																										// s in "Bill's
																										// clothes are
																										// great".
		} else {
			clause.constituents.add(new IndexedConstituent(newSemanticGraph, subject,
					Collections.<IndexedWord> emptySet(), excludesub, Type.SUBJECT));
		}
	 ...
	 * 
	 * @param outedobj
	 *            relations to be examined for exclusion
	 * @param excludeobj
	 *            The vertexes to be excluded
	 */
	private static void excludeVertexPoss(List<SemanticGraphEdge> outedobj, Set<IndexedWord> excludeobj,
			ClausIE clausIE) {
		for (SemanticGraphEdge ed : outedobj) {
			if (DpUtils.isAdvcl(ed) || DpUtils.isAdvmod(ed) || DpUtils.isAnyObj(ed) // currently everything is
					...
					|| DpUtils.isAnyConj(ed) && clausIE.options.processCcNonVerbs)
	 ...
	 * 
	 * @param root
	 *            Head of the parataxis relation
	 * @param parroot
	 *            Dependent of the parataxis relation
	 * @param roots
	 *            List of clause roots
	 */
	private static void addParataxisClause(ClausIE clausIE, IndexedWord root, IndexedWord parroot,
		...
		SemanticGraphEdge subject = DpUtils.findFirstOfRelationOrDescendent(outedges,
				EnglishGrammaticalRelations.SUBJECT);
		if (subject != null) {
			Constituent subjectConst = new IndexedConstituent(clausIE.semanticGraph, subject.getDependent(),
					Type.SUBJECT);
