 ...
 * 
 * Each constituent has a root vertex. The root together with its the descendants form the constituent. In some cases,
 * additional vertexes need to be included or excluded; these vertexes are also recorded within this class.
 * 
 * Note that the {@link SemanticGraph} may or may not match the graph of the input sentences or the other constituents
 * of the same clause. For example, the semantic graphs are modified when processing of coordinating conjunctions.
 * 
...
public class IndexedConstituent extends Constituent {
	...
	/**
	 * The root vertex of this constituent in {@link #semanticGraph}. This vertex and all its descendants are part of
	 * the constituent (unless they appear in {@link #excludedVertexes}).
	 */
	protected IndexedWord root;

	/**
	 * Additional root vertexes that form this constituent. These vertexes and all their descendants are part of the
	 * constituent (unless they appear in {@link #excludedVertexes}).
	 */
	protected Set<IndexedWord> additionalVertexes;

	/**
	 * Vertexes that are excluded from this constituent. All descendants are excluded as well (unless they appear in
	 * {@link #root} or {@link additionalRoots}).
	 */
	 ...
	 * 
	 * @param semanticGraph
	 *            Semantic graph for this constituent ({@see #semanticGraph})
	 * @param root
	 *            The root vertex of this constituent ({@see {@link #root})
	 * @param additionalVertexes
	 *            Additional root vertexes that form this constituent ({@see {@link #additionalVertexes})
	 * @param excludedVertexes
	 *            Vertexes that are excluded from this constituent ({@see {@link #excludedVertexes})
	 * @param type
	...
	/**
	 * Returns additional root vertexes that form this constituent ({@see {@link #additionalVertexes}).
	 */
	...
	public boolean isPrepositionalPhrase() { // This is a mess, find other way of fixing. This is purelly heuristic. It
												// needs to know the semantic graph for the sentence after this is fixed
												// the member variable sentSemanticGraph can be removed
		List<IndexedWord> parents = semanticGraph.getParentList(root); // This is not the cleanest way semantics messed
																		// up. specially with the rel we cannot just
																		// check if the head is a preposition (return
																		// root.tag().equals("IN")) because the parser
																		// some times includes a preposition in the
	...
	public boolean isPrepPhrase() { // This is a mess, find other way of fixing. This is purelly heuristic. It
		// needs to know the semantic graph for the sentence after this is fixed
		// the member variable sentSemanticGraph can be removed
		List<IndexedWord> parents = semanticGraph.getParentList(root); // This is not the cleanest way semantics messed
		// up. specially with the rel we cannot just
	...
	/**
	 * Returns a copy of the semantic graph of this constituent in which all edges (from any included vertex) to
	 * excluded vertexes have been removed. Useful for proposition generation.
	 */
	...
	public String toString() {
		return "IndexedConstituent [root=" + root;
	}
}
