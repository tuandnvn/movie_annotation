 ...
 * 
 * Coordinating conjunctions are still a difficult issue for the parser and therefore the source of a significant loss
 * in precision by ClausIE.
 * 
...
public class ProcessConjunctions {
	...
	public static List<Constituent> processCC(Tree depTree, Clause clause, Constituent constituent, int index) {
	...
	private static List<Constituent> generateConstituents(Tree depTree, Clause clause, IndexedConstituent constituent,
			int index) {
		...
		result.add(copy);
		generateConstituents(copy.getSemanticGraph(), depTree, copy, copy.getRoot(), result, true);
		return result;
	...
	private static void generateConstituents(SemanticGraph semanticGraph, Tree depTree, IndexedConstituent constituent,
		...
		if (conjunct != null) {
			...
			if (pobj != null && pobj.getGovernor().lemma() != null && pobj.getGovernor().lemma().equals("between"))
				processCC = false;
		...
		for (SemanticGraphEdge edge : outedges) {
			if (DpUtils.isParataxis(edge) || DpUtils.isRcmod(edge) || DpUtils.isAppos(edge)
				...
				continue;// to avoid processing relative clauses and appositions which are included as an independent
							// clause in the clauses list of the sentence, also no dep in verbs are processed. To
							// reproduce the results of the paper comment this line and eliminate the duplicate
							// propositions that may be generated.
			if (DpUtils.isAnyConj(edge) && processCC) {
				boolean cont = false;
				for (SemanticGraphEdge c : conjunct) {
					if (c.getDependent().lemma().equals("&") && nextToVerb(depTree, root.index(),
							edge.getDependent().index(), c.getDependent().index())) {
						cont = true;
						break;
					}
				}
				...
				if (firstLevel) {
					generateConstituents(newSemanticGraph, depTree, newConstituent, newRoot, constituents, false);
				} else {
					generateConstituents(newSemanticGraph, depTree, constituent, newRoot, constituents, false);
				}
					...
					if (option.processCcAllVerbs || !notProcess) {
						...
						for (SemanticGraphEdge e : conjunct) {
							if (e.getDependent().index() > edge.getDependent().index())
								continue;
							if (nextToVerb(depTree, root.index(), edge.getDependent().index(),
									e.getDependent().index())) {
								toRemove.add(e);
								break;
							}
						}
	...
	/**
	 * Rewrites the graph so that each conjoint is independent from each other. They will be disconnected and each
	 * dependent correspondignly assigned
	 */
		...
		for (int i = 0; i < ccs.size(); i++) {
			for (int j = i + 1; j < ccs.size(); j++) {
				...
				for (SemanticGraphEdge ed : semanticGraph.getIncomingEdgesSorted(ccs.get(i))) {
					if (semanticGraph.getParents(ccs.get(j)).contains(ed.getGovernor()))
						continue;
				...
				for (SemanticGraphEdge ed : semanticGraph.getOutEdgesSorted(ccs.get(i))) {
					IndexedWord child = ed.getDependent();
					if (semanticGraph.getChildren(ccs.get(j)).contains(child))
						continue;
	...
	private static boolean shareAll(List<SemanticGraphEdge> outedges, Tree depTree, IndexedWord root,
			IndexedWord conj) {
		for (SemanticGraphEdge edge : outedges) {
