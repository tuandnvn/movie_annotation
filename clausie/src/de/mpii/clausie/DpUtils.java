...
/**
 * This class provides a set of utilities to work with {@link SemanticGraph} For details on the Dependency parser @see
 * <a href="nlp.stanford.edu/software/dependencies_manual.pdf">the Stanford Parser manual
 * 
 */
public class DpUtils {
	 ...
	 * 
	 * This method traverses the semantic graph starting from the given root. An edge is removed if (1) its child
	 * appears in <code>excludeVertexes</code>, (2) its relation appears in <code>excludeRelations</code>, or (3) the
	 * edge has the root as parent and its relation appears in <code>excludeRelationsTop</code>.
	 */
	 ...
	 * Removes some edges from the given semantic graph.
	 * 
	 * This method traverses the semantic graph starting from the given root. An edge is removed if its child appears in
	 * <code>excludeVertexes</code>.
	 */
	public static void removeEdges(SemanticGraph graph, IndexedWord root, Collection<IndexedWord> excludeVertexes) {
	 ...
	 * Removes some edges from the given semantic graph.
	 * 
	 * This method traverses the semantic graph starting from the given root. An edge is removed if its relation appears
	 * in <code>excludeRelations</code> or the edge has the root as parent and its relation appears in
	...
	/**
	 * Implementation for {@link #removeEdges(SemanticGraph, IndexedWord, Collection, Collection, Collection)}
	 */
	private static void subgraph(SemanticGraph graph, IndexedWord root, Collection<IndexedWord> excludeVertexes,
		...
		for (SemanticGraphEdge e : edges) {
			...
			if (excludeVertexes.contains(child) || excludeRelations.contains(e.getRelation())
				...
				subgraph(graph, child, excludeVertexes, excludeRelations, Collections.<GrammaticalRelation> emptySet(),
						edgesToRemove);
			}
		}
	}
	...
	public static Set<IndexedWord> exclude(SemanticGraph semanticGraph, Collection<GrammaticalRelation> rels,
			IndexedWord root) {
